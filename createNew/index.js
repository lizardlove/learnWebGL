...
function showError(msg) {
    console.log('\x1b[31m%s', `Error: ${msg}`)
    process.exit(0)
}
...
async function create(name, route, desc, tags) {
    ...
    const renderTemplate = (temp) => temp.replace(/\{NAME\}/g, name).replace(/\{ROUTE\}/g, route).replace(/\{DESC\}/g, desc).replace(/\{DATE\}/g, date).replace(/\{TAGS\}/g, tags)
    ...
    try {
        await copy(templatesPath, dirPath, options).on(copy.events.COPY_FILE_START, copyOperation => {
            console.log(`Gernerate ${copyOperation.desc}...`)
        })
    } catch (error) {
